...
function IncomingCall() {
  const [{incomingVoiceCall, socket, videoCall},dispatch] = useStateProvider();
  const acceptCall = ()=>{
    dispatch({
  ...
  const rejectCall = ()=>{
    socket.current.emit("rejcet-voice-call", {from:incomingVoiceCall.id})
    dispatch({
      type:reducerCases.END_CALL
    })
  }
  return (
    <div className="h-24 w-80 fixed bottom-8 mb-0 right-6 z-50 rounded-sm flex gap-5 items-center justify-start p-4 bg-conversation-panel-background text-white drop-shadow-2x1 border-icon-green border-2 py-14">
      <div>
